...
export default class PropertyCard extends LightningElement {
    ...
    connectedCallback() {
        if (this.propertyInfo) {
            this.isAvailable = this.propertyInfo.Status__c === 'Available';
            this.mapMarkers = [
                {
                    location: {
                        City: this.propertyInfo.City__c,
                        Country: this.propertyInfo.Country__c,
                        PostalCode: this.propertyInfo.Postal_Code__c,
                        State: this.propertyInfo.State__c,
                        Street: this.propertyInfo.Address__c,
                    },
                    value: this.propertyInfo.Id,
                    icon: 'standard:account',
                    title: this.propertyInfo.Property_Name__c,
                    description: this.propertyInfo.Description__c,
                }
            ];
        }
    }
    ...
    generatePDF() 
    {
        ...
        // Content
            ...
            doc.text('Agreed Monthly Rent: $'+this.propertyInfo.Rent__c, 10, 30);
